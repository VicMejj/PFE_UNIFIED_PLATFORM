...
from ai_services.services.chatbot_engine import SmartChatbotEngine
...
def _check_chatbot_access(
    auth_header: str | None,
) -> tuple[bool, int, str, list[str] | None]:
    """Check chatbot access with improved error handling and retry logic."""
    if not auth_header:
        ...
        try:
            ...
            if attempt == max_retries - 1:
                return (
                    ...
                    "The authentication service is taking too long. Please try again.",
                    None,
                )
    ...
    if response.status_code == 403:
        return (
            ...
            "Forbidden. You are not allowed to use the chatbot.",
            None,
        )
    if response.status_code != 200:
        return (
            ...
            "Failed to verify user role. Please try again later.",
            None,
        )
    ...
    try:
        ...
        return (
            False,
            status.HTTP_502_BAD_GATEWAY,
            "Invalid role verification response. Please try again.",
            None,
        )
...
def predict_turnover(request):
    ...
    try:
        ...
        try:
            ...
            user_id = None
        session_uuid = str(
            request.data.get("session_id") or request.data.get("session_uuid") or ""
        )
        prediction_score = float(
            result.get("risk_score", result.get("prediction_score", 0)) or 0
        )
        saved = None
        try:
            saved = TurnoverPrediction.objects.create(
                ...
                factors_analyzed=result.get(
                    "factors", result.get("factors_analyzed", {})
                )
                or {},
                user_id=user_id,
        ...
        payload = (
            TurnoverPredictionSerializer(saved).data
            if saved
            else {
                "employee_id": employee_id,
                "prediction_score": prediction_score,
        ...
        return Response(
            {"success": True, "data": payload, "user_id": user_id},
...
def predict_leave_approval_probability(request):
    try:
        ...
        return Response(
            {"success": True, "data": {"approval_probability": round(score, 3)}},
            status=status.HTTP_200_OK,
        )
    except Exception as e:
...
def recommend_benefits(request):
    try:
        ...
        recent = {
            str(item).lower()
            for item in (employee.get("recent_benefits") or [])
            if item
        }
        ...
        for benefit in benefits:
            ...
            score += min(
                0.25, overall_score / 100
            )  # Give high weight to the holistic score

            if any(
                term in name or term in description
                for term in ["health", "medical", "wellness", "insurance"]
            ):
                score += 0.12
            if any(
                term in name or term in description
                for term in ["training", "learning", "education", "course"]
            ):
                score += 0.08
            if any(
                term in name or term in description
                for term in ["remote", "home", "transport", "commute"]
            ):
                score += 0.06
            ...
            recommendations.append(
                {
                    ...
                    "status": status_text,
                    "gap_actions": gaps,
                    "estimated_months_to_qualify": max(0, int((24 - tenure) / 2))
                    if score < 0.75
                    else 0,
                    "reasoning": f"Your overall score of {overall_score} makes you a good candidate for this benefit."
                    if score >= 0.75
                    else "This benefit is promising but still needs policy alignment.",
        ...
        return Response(
            {"success": True, "data": recommendations}, status=status.HTTP_200_OK
        )
    except Exception as e:
    ...
    try:
        insights = {
            ...
            "anomaly_alerts": [
                {
                    "type": "attendance",
                    "message": "Three days of low attendance detected in sales team.",
                },
            ],
...
def assess_loan_risk(request):
    ...
    try:
        ...
        try:
            saved = LoanRiskAssessment.objects.create(
                ...
                probability_of_default=float(
                    result.get(
                        "probability_of_default", result.get("default_probability", 0)
                    )
                    or 0
                ),
                recommendation=str(
                    result.get("recommendation", result.get("risk_level", "unknown"))
                ),
                risk_tier=str(
                    result.get("risk_tier", result.get("risk_level", "unknown"))
                ),
                user_id=user_id,
                session_uuid=session_uuid,
                input_data=dict(request.data),
        ...
        payload = (
            LoanRiskAssessmentSerializer(saved).data
            if saved
            else {
                "loan_id": request.data.get("loan_id"),
                "employee_id": employee_id,
...
def chatbot_history(request):
    ...
    try:
        ...
        if session_id:
            ...
            query = ChatbotConversation.objects.filter(user_id=user_id).order_by(
                "-started_at"
            )
